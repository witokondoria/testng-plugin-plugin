...
public class TestParser {
    ...
    public void testTestngXmlWithNonExistingResultXml() {
        ...
        Assert.assertTrue("Collection should have been empty. Number of results : " + results.getTestList().size(),
    ...
    public void testParseEmptyException() {
        TestNGResult results = CommonUtil.getResults(CommonUtil.getResource(Constants.TESTNG_XML_EMPTY_EXCEPTION)
                .getFile());
        results.tally();
        Assert.assertEquals(1, results.getPassCount());
        MethodResult mr = results.getPassedTests().get(0);
        Assert.assertEquals("$java.lang.IllegalStateException$$EnhancerByMockitoWithCGLIB$$c0ded2d3", mr.getException()
                .getExceptionName());
    }
    ...
    public void testReporterOutputForMethods() throws ParseException {
        ...
        Assert.assertEquals("Some Reporter.log() statement<br/>Another Reporter.log() statement<br/>", results
